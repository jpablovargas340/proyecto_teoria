...
st.set_page_config(
    page_title="Teoría del Riesgo · Dashboard",
    page_icon="📈",
    layout="wide",
...
apply_custom_style()
hero(
    "Teoría del Riesgo — Dashboard Profesional",
    "Análisis técnico · Rendimientos · GARCH · CAPM · VaR/CVaR · Markowitz · Señales · Benchmark"
)
...
# ────────────────────────────────────────────────
with st.sidebar:
    st.markdown(f"### ⚙️ Configuración general")
    ...
    portfolio_value = st.number_input("Valor del portafolio (USD)", min_value=1000.0,
    ...
    if rf_override_enabled:
        rf_override = st.number_input("Rf anual", min_value=0.0, max_value=0.40,
...
with tabs[0]:
    ...
    info_box(
        "Lectura sugerida",
        "Este tablero integra análisis técnico, distribución de rendimientos, riesgo extremo, "
        "volatilidad condicional, CAPM y optimización. Secuencia recomendada: Resumen → "
        "Técnico → Rendimientos → GARCH → CAPM → VaR/CVaR → Markowitz → Pesos → Señales → Macro."
    )

    st.plotly_chart(
        plot_cumulative_vs_benchmark(optimizer_10k["max_sharpe_returns"], market_returns),
        use_container_width=True, key="cum_bench_res"
    )
...
with tabs[1]:
    ...
    st.plotly_chart(plot_price_technical(prices[ticker].dropna(), pack),
                    use_container_width=True, key="price_tech")

    c1, c2 = st.columns(2)
    c1.plotly_chart(plot_rsi(pack["rsi"]), use_container_width=True, key="rsi_ch")
    c2.plotly_chart(plot_macd(pack["macd"], pack["macd_signal"], pack["macd_hist"]),
                    use_container_width=True, key="macd_ch")

    c3, c4 = st.columns(2)
    c3.plotly_chart(plot_stochastic(pack["stochastic_k"], pack["stochastic_d"]),
                    use_container_width=True, key="stoch_ch")
    with c4:
        st.markdown("##### Panel interpretativo")
        interp = indicator_interpretations(pack, prices[ticker].dropna().iloc[-1],
                                           rsi_upper, rsi_lower)
...
with tabs[3]:
    ...
    if greport["cond_vol"] is not None:
        c1.plotly_chart(plot_garch_conditional_vol(greport["cond_vol"], greport["forecast"]),
                        use_container_width=True, key="garch_vol")
    if greport["std_resid"] is not None:
        c2.plotly_chart(plot_garch_residuals(greport["std_resid"]),
                        use_container_width=True, key="garch_res")

    info_box("Diagnóstico", greport["interpretation"])
    if greport["residual_jb"] is not None:
        st.write(f"**Jarque-Bera residuos:** estadístico={greport['residual_jb'][0]:.4f}, "
                 f"p-valor={greport['residual_jb'][1]:.6f}")

# ── TAB 4: CAPM ──────────────────────────────────
with tabs[4]:
    section_title("CAPM y riesgo sistemático", "📉")
    capm_rows = [capm_report(log_returns[t].dropna(), market_returns, macro["risk_free_rate"], t)
                 for t in log_returns.columns]
    capm_df = pd.DataFrame(capm_rows)
    st.dataframe(
        format_pct_df(capm_df[["ticker","beta","alpha_anual","r_squared",
                                "expected_return","classification"]],
    ...
    st.plotly_chart(plot_capm_scatter(selected["scatter_df"], ticker, selected["beta"]),
                    use_container_width=True, key="capm_scat")
    info_box("Interpretación", selected["interpretation"])
    info_box("Marco teórico",
             "La beta mide riesgo sistemático — la porción del riesgo que no puede eliminarse "
             "mediante diversificación. El riesgo no sistemático se reduce construyendo "
...
with tabs[5]:
    ...
    st.dataframe(format_pct_df(rrep["table"], pct_cols=["VaR %","VaR anual %"]),
                 use_container_width=True)
    st.plotly_chart(plot_var_distribution(log_returns[ticker].dropna(), rrep),
                    use_container_width=True, key="var_dist")
    info_box("Interpretación", rrep["interpretation"])
    st.write(f"**Backtesting de Kupiec:** violaciones esperadas={rrep['kupiec']['expected_violations']:.2f}, "
...
with tabs[6]:
    ...
    st.plotly_chart(
        plot_efficient_frontier(optimizer_10k["simulated"],
                                optimizer_10k["max_sharpe"],
                                optimizer_10k["min_variance"]),
        use_container_width=True, key="eff_front"
    )
    ...
    st.markdown("---")
    target = st.slider("Rendimiento objetivo anual", 0.0, 0.50, 0.15, 0.01,
                        format="%.0f%%")
...
with tabs[7]:
    ...
    info_box(
        ...
        "Aquí puedes definir manualmente los pesos de cada activo, compararlos con los "
        "portafolios óptimos de Markowitz y ver en tiempo real las métricas de riesgo-retorno "
        "del portafolio que estás construyendo."
    )
    ...
    st.markdown("#### Ajusta los pesos de cada activo")
    ...
    # Calcular métricas del portafolio manual
    w_arr = np.array([manual_weights[a] for a in assets])
    man_ret, man_risk, man_sharpe = portfolio_performance(
        w_arr, mean_returns_a, cov_matrix_a, macro["risk_free_rate"]
    )
    manual_port = {"return": man_ret, "risk": man_risk, "sharpe": man_sharpe,
                   "weights": manual_weights}

    st.markdown("---")
    st.markdown("#### 📊 Resultados del portafolio configurado")
    render_portfolio_kpis(manual_port)

    col_donut, col_table = st.columns([1, 1])
    with col_donut:
        st.plotly_chart(
            plot_portfolio_weights_donut(manual_weights, "Distribución de pesos — Manual"),
            use_container_width=True, key="donut_manual"
        )
    with col_table:
        st.markdown("<br>", unsafe_allow_html=True)
        render_weights_table(manual_weights)

    # ── Comparación con portafolios óptimos
    st.markdown("---")
    st.markdown("#### 🔍 Comparación con portafolios de Markowitz")

    comp_data = {
        "Portafolio": ["📐 Manual", "⭐ Máx. Sharpe", "💎 Mín. Varianza"],
        "Retorno esperado": [man_ret,
    ...
    fig_comp.add_trace(__import__("plotly.graph_objects", fromlist=["Scatter"]).Scatter(
        x=[man_risk], y=[man_ret], mode="markers+text",
        marker=dict(size=18, symbol="circle", color="#F85149",
                    line=dict(color="#0D1117", width=2)),
        name="Tu portafolio",
        text=["Tu portafolio"], textposition="top right",
        textfont=dict(color="#F85149", size=11),
    ))
    fig_comp.update_layout(title="Tu portafolio en la frontera eficiente")
    ...
    with cd1:
        st.plotly_chart(
            plot_portfolio_weights_donut(manual_weights, "Tu portafolio"),
            use_container_width=True, key="donut_man2"
        )
    with cd2:
        st.plotly_chart(
            plot_portfolio_weights_donut(optimizer_10k["max_sharpe"]["weights"], "Máx. Sharpe"),
            use_container_width=True, key="donut_sharpe"
        )
    with cd3:
        st.plotly_chart(
            plot_portfolio_weights_donut(optimizer_10k["min_variance"]["weights"], "Mín. Varianza"),
            use_container_width=True, key="donut_minvar"
        )
...
with tabs[9]:
    ...
    st.plotly_chart(
        ...
        use_container_width=True, key="cum_macro"
    )
